...
contract PipelineContract {
	...
	modifier onlyPipelineOwner(bytes32 _pipelineId) {
		require(
			pipelines[_pipelineId].owner == msg.sender,
			'You can only modify pipelines you created'
		);
		_;
	}
	 ...
	 */
	function createPipeline(string calldata _jsContractAddress) external {
		bytes32 _pipelineId = keccak256(
			abi.encodePacked(msg.sender, _jsContractAddress)
		);
		// validate pipeline logic
		require(
			!pipelines[_pipelineId].initialised,
			'Please supply another pipelineId'
		);
	...
	function addSourceToPipeline(
		...
		// validate pipeline logic
		require(
			existingPipeline.owner == msg.sender,
			'You can only modify pipelines created by you!'
		);

		// create new source and add to pipeline
		Source memory newSource = Source(
			_sourceType,
			_sourceContractAddress,
			_eventType,
			_sourceContractABI
		);
		...
		returns (SupportedSources, string memory, string memory, string memory)
	{
		...
		return (
			...
			existingSource.sourceContractAddress,
			existingSource.eventType,
			existingSource.sourceContractABI
		);
	}
