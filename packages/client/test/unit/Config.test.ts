...
describe('Config', () => {
	describe('validate', () => {
		...
		it('empty array', () => {
			expect(() => {
				...
				'/contracts/mainChainRPCs/rpcs must NOT have fewer than 1 items'
			);
		});
	...
	describe('ignorable properties', () => {
		it('auth address', () => {
			expect(() => {
				...
				return new LogStoreClient({ auth: wallet });
			}).not.toThrow();
		});
	});
	...
	describe('merging configs', () => {
		it('works with no arguments', () => {
			expect(new LogStoreClient()).toBeInstanceOf(LogStoreClient);
		});
		...
		it('can override network.trackers arrays', () => {
			...
			expect(clientOverrides.network.trackers).not.toEqual(
				clientDefaults.network.trackers
			);
		...
		it('can override trackers', () => {
			...
			expect(
				(clientOverrides.network.trackers as TrackerRegistryRecord[])[0]
			).not.toBe(trackers[0]);
		});
	});
