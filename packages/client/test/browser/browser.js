/* eslint-disable no-undef, @typescript-eslint/no-require-imports, @typescript-eslint/no-var-requires */
...
describe('LogStoreClient', () => {
	...
	before(async (browser) => {
		...
		const url = process.env.WEBSOCKET_URL
			? `&WEBSOCKET_URL=${encodeURIComponent(process.env.WEBSOCKET_URL)}`
			: '';
	...
	test('Test LogStoreClient in Chrome Browser', (browser) => {
		...
		browser
			...
			.assert.containsText(
				'#result',
				'0xde1112f631486CfC759A50196853011528bC5FA0'.toLowerCase()
			)
			.assert.not.elementPresent('.error')
