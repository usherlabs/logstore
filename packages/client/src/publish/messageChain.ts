...
export const createMessageRef = (
	...
	const isBackdated =
		prevMsgRef !== undefined && timestamp < prevMsgRef.timestamp;
	if (isBackdated) {
		throw new Error('prevMessageRef must come before current');
	}
	const isSameTimestamp =
		prevMsgRef !== undefined && prevMsgRef.timestamp === timestamp;
	const nextSequenceNumber = isSameTimestamp
		? prevMsgRef!.sequenceNumber + 1
		: 0;
