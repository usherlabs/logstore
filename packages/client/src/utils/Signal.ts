import { pLimitFn, pOnce, pOne } from './promises';
...
export class Signal<ArgsType extends any[] = []> {
	 ...
	 *  See example above.
	 */
	static create<ArgsType extends any[] = []>(
	...
	protected listeners: (
		| SignalListener<ArgsType>
		| SignalListenerWrap<ArgsType>
	)[] = [];
	...
	unlisten(cb: SignalListener<ArgsType>): this {
		const index = this.listeners.findIndex((listener) => {
			return (
				listener === cb || ('listener' in listener && listener.listener === cb)
			);
		});
...
export class ErrorSignal<
	ArgsType extends [Error] = [Error]
> extends Signal<ArgsType> {
