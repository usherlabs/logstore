...
export class GroupKey {
	...
	constructor(groupKeyId: string, data: Buffer) {
		...
		if (!data) {
			throw new GroupKeyError(
				`groupKeyBufferOrHexString must not be falsey ${data}`
			);
		}
		this.data = data;
	...
	private static validate(maybeGroupKey: GroupKey): void | never {
		if (!maybeGroupKey) {
			throw new GroupKeyError(
				`value must be a ${this.name}: ${maybeGroupKey}`,
				maybeGroupKey
			);
		}
		...
		if (!maybeGroupKey.id || typeof maybeGroupKey.id !== 'string') {
			throw new GroupKeyError(
				`${this.name} id must be a string: ${maybeGroupKey}`,
				maybeGroupKey
			);
		}
		...
		if (!maybeGroupKey.data || !Buffer.isBuffer(maybeGroupKey.data)) {
			throw new GroupKeyError(
				`${this.name} data must be a Buffer: ${maybeGroupKey}`,
				maybeGroupKey
			);
		}
		if (maybeGroupKey.data.length !== 32) {
			throw new GroupKeyError(
				`Group key must have a size of 256 bits, not ${
					maybeGroupKey.data.length * 8
				}`,
				maybeGroupKey
			);
		}
	...
	encryptNextGroupKey(nextGroupKey: GroupKey): EncryptedGroupKey {
		return new EncryptedGroupKey(
			nextGroupKey.id,
			EncryptionUtil.encryptWithAES(nextGroupKey.data, this.data)
		);
	}
	...
	decryptNextGroupKey(nextGroupKey: EncryptedGroupKey): GroupKey {
		return new GroupKey(
			...
			EncryptionUtil.decryptWithAES(
				nextGroupKey.encryptedGroupKeyHex,
				this.data
			)
		);
	}
	...
	/** @internal */
	static decryptRSAEncrypted(
		encryptedKey: EncryptedGroupKey,
		rsaPrivateKey: string
	): GroupKey {
		return new GroupKey(
			encryptedKey.groupKeyId,
			EncryptionUtil.decryptWithRSAPrivateKey(
				encryptedKey.encryptedGroupKeyHex,
				rsaPrivateKey,
				true
			)
		);
