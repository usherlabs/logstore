...
export class EncryptionUtil {
	private static validateRSAPublicKey(publicKey: crypto.KeyLike): void | never {
		const keyString =
			typeof publicKey === 'string' ? publicKey : publicKey.toString('utf8');
		if (
			...
			throw new Error(
				'"publicKey" must be a PKCS#8 RSA public key in the PEM format'
			);
		}
	}
	...
	static decryptWithRSAPrivateKey(
		...
		const ciphertextBuffer = isHexString
			? arrayify(`0x${ciphertext}`)
	...
	static encryptWithAES(data: Uint8Array, cipherKey: CipherKey): string {
		...
		return (
			hexlify(iv).slice(2) +
			cipher.update(data, undefined, 'hex') +
			cipher.final('hex')
		);
	}
	...
	static decryptWithAES(ciphertext: string, cipherKey: CipherKey): Buffer {
		...
		return Buffer.concat([
			decipher.update(ciphertext.slice(32), 'hex'),
			decipher.final(),
		]);
	}
	...
	static decryptStreamMessage(
		...
		try {
			...
			const serializedContent = this.decryptWithAES(
		...
		try {
			...
			throw new DecryptError(
				streamMessage,
				'Could not decrypt new group key: ' + err.stack
			);
		}
