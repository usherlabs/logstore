...
type ChainedMessage = StreamMessage & {
	prevMsgRef: NonNullable<StreamMessage['prevMsgRef']>;
};
...
export const MsgChainEmitter = EventEmitter as {
...
class OrderedMsgChain extends MsgChainEmitter {
	...
	private hasNextMessageInChain(): boolean {
		...
		if (prevMsgRef !== null) {
			...
			return (
				streamMessage.getMessageRef().compareTo(this.lastOrderedMsgRef) > 0
			);
		}
	}
	...
	private async requestGapFill(): Promise<void> {
		...
		const from = new MessageRef(
			lastOrderedMsgRef.timestamp,
			lastOrderedMsgRef.sequenceNumber + 1
		);
		const { gapRequestCount, maxGapRequests } = this;
		if (gapRequestCount < maxGapRequests) {
			logger.trace(
				'requestGapFill %d of %d: %j',
				gapRequestCount + 1,
				maxGapRequests,
				{
					from,
	...
	private onGapFillsExhausted() {
		...
		if (this.isGapHandlingEnabled()) {
			logger.trace(
				'requestGapFill failed after %d attempts: %o',
				maxGapRequests,
				{
					from,
					to,
