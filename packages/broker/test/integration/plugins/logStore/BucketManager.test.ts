...
describe('BucketManager', () => {
	...
	const insertBuckets = async (startTimestamp: Date) => {
		for (let i = 0; i < 100; i++) {
			const currentTimestamp = new Date(
				startTimestamp.getTime() + i * 60 * 1000
			); // + "i" minutes
	...
	test('calling getBucketId() will try to find bucket in database and then create if not found and store in database', async () => {
		...
		let result = await cassandraClient.execute(
			'SELECT * FROM bucket WHERE stream_id = ? ALLOW FILTERING',
			[streamId]
		);
		...
		await waitForCondition(
			() => bucketManager.buckets[foundBucketId].isStored() === true
		);
	...
	test('calling getBucketId() updates last know min timestamp and resets it to undefined when bucket is found', async () => {
		...
		expect(bucketManager.getBucketId(streamId, 0, timestamp)).toBeUndefined();
		expect(bucketManager.streamParts[`${streamId}-0`].minTimestamp).toEqual(
			timestamp
		);

		await waitForCondition(
			() => bucketManager.getBucketId(streamId, 0, timestamp) !== undefined
		);
		expect(
			bucketManager.streamParts[`${streamId}-0`].minTimestamp
		).toBeUndefined();

		// future timestamp will give latest not full bucket
		expect(
			bucketManager.getBucketId(streamId, 0, timestamp + 600)
		).not.toBeUndefined();
		expect(
			bucketManager.streamParts[`${streamId}-0`].minTimestamp
		).toBeUndefined();
	...
	test('calling getBucketId() with timestamp in the past, will try to find correct bucket and then create buckets in the past', async () => {
		...
		expect(
			bucketManager.getBucketId(streamId, 0, timestamp5ago)
		).toBeUndefined();
		await waitForCondition(
			() => bucketManager.getBucketId(streamId, 0, timestamp5ago) !== undefined
		);
		const bucketId5minAgo = bucketManager.getBucketId(
			streamId,
			0,
			timestamp5ago
		)!;
		...
		await waitForCondition(() =>
			bucketManager.buckets[lastBucketId].isStored()
		);
		await waitForCondition(() =>
			bucketManager.buckets[bucketId5minAgo].isStored()
		);
	...
	test('buckets are removed from memory after opts.bucketKeepAliveSeconds', async () => {
		...
		await waitForCondition(
			() =>
				bucketManager.getBucketId(streamId, 0, latestTimestamp) !== undefined
		);
		...
		await waitForCondition(
			() => Object.values(bucketManager.buckets).length === 0
		);
	}, 10000);
});
