...
describe('ConfigWizard', () => {
	...
	describe('plugin port validation', () => {
		...
		it('invalid data: out-of-range number', () => {
			...
			expect(validate(port)).toBe(
				`Out of range port ${port} provided (valid range 1024-49151)`
			);
		});
	...
	describe('createStorageFile', () => {
		...
		beforeAll(() => {
			tmpDataDir = mkdtempSync(
				path.join(os.tmpdir(), 'broker-test-config-wizard')
			);
		});
		...
		it('happy path; create directories if needed', async () => {
			...
			const configFileLocation: string = await createLogStoreFile(CONFIG, {
				logStorePath: configPath,
			});
	...
	describe('getConfig', () => {
		const assertValidPort = (
			port: number | string,
			pluginName = 'websocket'
		) => {
		...
		it('should exercise the happy path with user-provided data', () => {
			...
			const config = getConfig(MOCK_PRIVATE_KEY, pluginsAnswers);
			expect(config.plugins.websocket.port).toBe(
				parseInt(pluginsAnswers.websocketPort!)
			);
	...
	describe('identity', () => {
		it('happy path', () => {
			...
			expect(identity.networkExplorerUrl).toBe(
				'https://streamr.network/network-explorer/nodes/0x909DC59FF7A3b23126bc6F86ad44dD808fd424Dc'
			);
		});
	});

	describe('user flow', () => {
		const assertValidFlow = async (
			pluginAnswers: PluginAnswers,
			assertPluginConfig: (config: any) => void
		) => {
			const tmpDataDir = mkdtempSync(
				path.join(os.tmpdir(), 'broker-test-config-wizard')
			);
			...
			const storageAnswers: LogStoreAnswers = {
				logStorePath: tmpDataDir + 'test-config.json',
			};
			...
			expect(logger.messages).toEqual([
				...
				`streamr-broker ${storageAnswers.logStorePath}`,
			]);
			const fileContent = readFileSync(storageAnswers.logStorePath).toString();
			const config = JSON.parse(fileContent);
			expect(config.client.auth.privateKey).toBe(
				privateKeyAnswers.importPrivateKey
			);
