...
const checkDBCount = async (cassandraClient: Client, streamId: string) => {
	const countBuckets =
		'SELECT COUNT(*) FROM bucket WHERE stream_id = ? AND partition = 0 ALLOW FILTERING';
	const bucketResult = await cassandraClient.execute(countBuckets, [streamId], {
		prepare: true,
	});
	const countData =
		'SELECT COUNT(*) FROM stream_data WHERE stream_id = ? AND partition = 0 ALLOW FILTERING';
	...
	daysArray.map(async (days) => {
		test(
			...
			async () => {
				...
				const bucketId1 = await insertBucket(
					cassandraClient,
					streamId,
					now - 0 * DAY_IN_MS
				);
				const bucketId2 = await insertBucket(
					cassandraClient,
					streamId,
					now - 1 * DAY_IN_MS
				);
				const bucketId3 = await insertBucket(
					cassandraClient,
					streamId,
					now - 2 * DAY_IN_MS
				);
				const bucketId4 = await insertBucket(
					cassandraClient,
					streamId,
					now - 3 * DAY_IN_MS
				);
