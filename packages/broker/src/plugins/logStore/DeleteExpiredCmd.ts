...
const totalSizeOfBuckets = (buckets: BucketInfo[]) =>
	buckets.reduce((mem, { size }) => mem + size, 0) / (1024 * 1024);

const totalNumOfRecords = (buckets: BucketInfo[]) =>
	buckets.reduce((mem, { records }) => mem + records, 0);
...
export class DeleteExpiredCmd {
	...
	async run(client: StreamrClient): Promise<void> {
		...
		const potentialBuckets = await this.getPotentiallyExpiredBuckets(
			streamsInfo
		);
		logger.info(
			'Found %d potentially expired buckets',
			potentialBuckets.length
		);

		const cutPotentialBuckets = potentialBuckets.slice(0, this.bucketLimit);
		logger.info(
			'Left with %d potentially expired buckets after cutting',
			cutPotentialBuckets.length
		);

		const expiredBuckets = await this.filterExpiredBuckets(cutPotentialBuckets);
		logger.info(
			'Found %d expired buckets (total records %d and size %d MB)',
	...
	private async getPotentiallyExpiredBuckets(
		streamsInfo: (StreamPartInfo | undefined)[]
	): Promise<BucketInfo[]> {
		const result: BucketInfo[] = [];

		const query =
			'SELECT * FROM bucket WHERE stream_id = ? AND partition = ? AND date_create <= ?';
	...
	private async filterExpiredBuckets(
		potentialBuckets: BucketInfo[]
	): Promise<BucketInfo[]> {
		const result: BucketInfo[] = [];
	...
	private async deleteExpired(
		...
		const tasks = expiredBuckets.filter(Boolean).map((stream) => {
			...
			const queries = [
				{
					query:
						'DELETE FROM bucket WHERE stream_id = ? AND partition = ? AND date_create = ?',
					params: [streamId, partition, dateCreate],
				},
				{
					query:
						'DELETE FROM stream_data WHERE stream_id = ? AND partition = ? AND bucket_id = ?',
					params: [streamId, partition, bucketId],
				},
			];

			logger.info(
				'Deleting expired bucket [%s, %d, %s]',
				streamId,
				partition,
				bucketId
			);
