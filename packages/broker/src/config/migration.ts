...
export const needsMigration = (config: any): boolean => {
	return (
		getVersion(config) !== undefined &&
		getVersion(config) !== CURRENT_CONFIGURATION_VERSION
	);
};
...
const convertV1ToV2 = (source: any): ConfigFile => {
	...
	const consoleAndPM2IntervalInSeconds =
		source.plugins.metrics?.consoleAndPM2IntervalInSeconds;
	if (
		consoleAndPM2IntervalInSeconds !== undefined &&
	...
	const isMetricsPluginEnabled =
		source.plugins.metrics !== undefined &&
		source.plugins.metrics.nodeMetrics !== null;
	...
	if (target.plugins.logStore !== undefined) {
		deleteNullProperties(target.plugins.logStore.cluster);
	}
	if (target.plugins.websocket !== undefined) {
		deleteNullProperties(target.plugins.websocket);
	}
	if (
		target.httpServer?.certFileName !== undefined ||
		target.httpServer?.privateKeyFileName !== undefined
	) {
...
export const readConfigAndMigrateIfNeeded = (
	...
	if (fileName === undefined) {
		...
		fileName = [defaultTargetFile, legacyTargetFile].find((file) =>
			fs.existsSync(file)
		);
		if (fileName === undefined) {
			/*
	...
	if (needsMigration(content)) {
		if (explicitTargetFile === undefined) {
			...
			console.log(
				`Migrating config ${fileName}, saving backup to ${backupFile}`
			);
			...
			fs.chmodSync(
				explicitTargetFile,
				(fs.statSync(fileName).mode & parseInt('777', 8)).toString(8)
			);
