...
export function validateIsNotNegativeInteger(
	varName: string,
	varValue: unknown,
	allowNull = false
): void | never {
	...
	if ((varValue as number) < 0) {
		throw new ValidationError(
			`Expected ${varName} to not be negative (${varValue}).`
		);
	}
}
...
export function validateIsArray(
	...
	if (!Array.isArray(varValue)) {
		throw new ValidationError(
			`Expected ${varName} to be an array but was a ${typeof varValue} (${varValue}).`
		);
	}
}
...
export function validateIsType(
	...
	if (!(varValue instanceof (typeClass as any))) {
		const msg = `Expected ${varName} to be an instance of (${typeName}), but it was: ${JSON.stringify(
			varValue
		)}`;
...
export function validateIsOneOf(
	...
	if (!validValues.includes(varValue)) {
		const msg = `Expected ${varName} to be one of ${JSON.stringify(
			validValues
		)} but was (${varValue}).`;
		throw new ValidationError(msg);
	}
