...
export const getEndpointUrl = (
	...
	return (
		baseUrl + '/' + pathParts.map((part) => encodeURIComponent(part)).join('/')
	);
};

export function formStorageNodeAssignmentStreamId(
...
export const formLookupKey = <K extends (string | number)[]>(
	...args: K
): string => {
	return args.join('|');
};
